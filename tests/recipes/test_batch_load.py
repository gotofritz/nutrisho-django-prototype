...
def gotofritz(db):
    ...
    user, _ = DjangoUser.objects.get_or_create(username="gotofritz", defaults={"password": "x"})
    return user
...
def test_dry_run_no_db_changes(gotofritz, tmp_path):
    ...
    call_command("batch_load_yaml_recipes", str(tmp_path), dry_run=True)
...
def test_failed_import_does_not_corrupt_existing_recipe(gotofritz, tmp_path):
    ...
    call_command("batch_load_yaml_recipes", str(p))
...
@pytest.mark.django_db
def test_dry_run_works_without_gotofritz_user(tmp_path):
    """--dry-run should not require gotofritz user to exist in DB."""
    # no gotofritz fixture — User table is empty
...
@pytest.mark.django_db
def test_dry_run_catches_invalid_quantity(tmp_path):
...
@pytest.mark.django_db
def test_dry_run_catches_invalid_serves(tmp_path):
...
def test_shared_tag_not_removed_from_first_recipe(gotofritz, tmp_path):
    ...
    call_command("batch_load_yaml_recipes", str(tmp_path / "a.yml"))

    data_b = make_yaml(title="Recipe B")
    data_b["tags"] = ["vegetarian"]
    write_yaml(tmp_path, data_b, "b.yml")
    call_command("batch_load_yaml_recipes", str(tmp_path / "b.yml"))
...
def test_import_always_inserts_never_updates(gotofritz, tmp_path):
    ...
    call_command("batch_load_yaml_recipes", str(p1))
    call_command("batch_load_yaml_recipes", str(p2))
...
def test_missing_only_skips_when_sanitized_export_already_exists(gotofritz, tmp_path):
    ...
    call_command("export_recipes_to_yaml", str(out_dir), missing_only=True)
...
def test_dry_run_catches_duplicate_recipe_name(gotofritz, tmp_path):
    ...
    call_command("batch_load_yaml_recipes", str(p))  # real insert

    with pytest.raises(CommandError, match="already exist"):
        call_command("batch_load_yaml_recipes", str(p), dry_run=True)


@pytest.mark.django_db
def test_dry_run_catches_duplicate_title_within_batch(tmp_path):
...
def test_import_raises_on_yaml_filename_collision_within_batch(gotofritz, tmp_path):
    ...
    with pytest.raises(CommandError, match="[Ff]ilename"):
        call_command("batch_load_yaml_recipes", str(dir_a / "soup.yml"), str(dir_b / "SOUP.yml"))
...
def test_import_raises_on_yaml_filename_collision_with_existing_recipe(gotofritz, tmp_path):
    ...
    call_command("batch_load_yaml_recipes", str(dir_a / "soup.yml"))

    with pytest.raises(CommandError, match="[Ff]ilename"):
        call_command("batch_load_yaml_recipes", str(dir_b / "SOUP.yml"))
