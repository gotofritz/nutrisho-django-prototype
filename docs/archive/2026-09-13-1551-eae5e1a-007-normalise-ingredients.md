...
## Status: Done (Phases 1–7) — archived 2026-09-13

Finalised 2026-09-13. Phases 1–7 shipped; Phase 8 (alias layer) was always scoped
to a separate PR and is carried forward as `docs/plans/008-ingredient-aliases.md`.
The constraints below were re-verified against the codebase at `12caf31`.

See **Outcome** at the end of this file for what changed during implementation.
...
## Outcome (Phases 1–7, 2026-09-13)

Shipped as planned. Worth recording where implementation differed or went further
than the plan spelled out:

- **Column 3 counts substitute references.** Decision 4 says the point is to see
  everything a clean-up would touch. Merge and delete repoint `IngredientInRecipe`
  `.substitute` as well as `.ingredient`, so a recipe that only names an ingredient
  as a substitute *is* touched, and is listed.
- **The delete blast radius is counted across all owners.** Column 3 stays
  owner-scoped as specified, but `Ingredient` rows are global, so the confirmation
  panel counts recipes the current user cannot see. Anything else would understate
  what the delete does.
- **No client-side JavaScript.** The plan expected buttons to be disabled
  client-side. Because ticking a checkbox already round-trips (to refresh column 3),
  the response also swaps the column-2 header and the column-1 buttons out of band,
  so all of it is server-rendered. Same for "the clicked button becomes Cancel".
- **`hx-include` has to name the inputs.** Pointing it at a container element
  (`this`, `#col-ingredients`, `#workspace-selection`) sends no values at all. The
  templates use `.ingredient-pick:checked` and `.workspace-pick`, as
  `_groups_list.html` already did. Django tests could not catch this — it only
  showed up driving the page in a browser.
- **7.1 and 7.2 are in tension.** Once 7.2's constraint exists, no database can hold
  the `Onion`/`onion` pair 7.1's test needs. The test fixture drops the unique index
  for the duration of the test, which is exactly the pre-migration state the command
  exists to serve.
- **Pre-existing blocker fixed on the way.** `base.html` loaded
  `django_htmx/htmx.min.js`, which the installed django-htmx does not ship (it ships
  `htmx-2.min.js` / `htmx-4.min.js`). htmx never loaded, so every `hx-*` attribute in
  the app was inert — inline editing included — with no error anywhere. Pinned to the
  2.x bundle and covered by a staticfiles-finder test.
