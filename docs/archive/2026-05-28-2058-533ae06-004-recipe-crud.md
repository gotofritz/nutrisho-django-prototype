...
## Completed in PR #13 (fix/list-delete-htmx)

- ✅ Step 1: URL scheme — all HTMX endpoints wired; ingredient-add scoped to group via `groups/<id>/ingredients/add/`
- ✅ Step 2: Views — field, step, ingredient, group inline-edit views
- ✅ Step 3: Forms — `StepForm` (textarea), `IngredientInRecipeForm`, `IngredientGroupForm`, `RecipeFieldForm`
- ✅ Step 4: Inline edit UX — click-to-edit on all fields, steps, ingredients, groups; `_group_block.html` partial for full group+list render
- ✅ Step 5: List page — New Recipe button, per-row delete with DOM removal via HTMX
- ⬜ Step 6 (partial): CSS list numbering restored; quantity trailing zeros stripped; `sql_to_edit_recipes_hack/` and unused `/update` route left for future cleanup PR

## Deferred

- Auth: CRUD remains open (single-user prototype, deferred to separate plan)
- Servings adjuster: implemented as plain text field; +/- widget deferred
